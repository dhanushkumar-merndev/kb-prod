-- Monthly salary is paid for the calendar month, not only for days attended, so
-- weekly offs and holidays no longer dock pay. A day costs a monthly employee
-- salary only when approved leave falls on it and paid leave is not enabled, or
-- when attendance explicitly records the day as absent. Approved paid leave
-- always wins over an absence marker on the same day.
create or replace function public.payroll_day_is_unpaid(
...
as $$
  select case
    when exists (
      ...
      select st.paid_leave
      from public.payroll_salary_structures st
      where st.profile_id = p_profile_id
        and st.effective_from <= p_day
  ...
  update public.payroll_entries pe set
    attendance_days = (select count(distinct s.shift_date) from public.attendance_shifts s
      where s.organization_id = pe.organization_id
        ...
        and s.shift_date between p_period_start and p_period_end),
    payable_days = case
      -- Monthly staff are salaried for the whole calendar month, so rest days and
      -- holidays stay payable. Only recorded absence and approved unpaid leave
      -- reduce the count.
      when (select p.payment_type from public.profiles p where p.id = pe.profile_id) = 'monthly'
      then (select count(*)
        from generate_series(p_period_start::timestamp, p_period_end::timestamp, interval '1 day') d
        join public.profiles p on p.id = pe.profile_id
        where d::date >= coalesce(p.joining_date, p_period_start)
          and not public.payroll_day_is_unpaid(pe.organization_id, pe.profile_id, d::date))
      -- Daily, hourly and temporary workers are paid only for days actually worked.
      else (select count(*) from generate_series(p_period_start::timestamp, p_period_end::timestamp, interval '1 day') d
